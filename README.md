...
| `kill PID` | Graceful-then-forced termination of a **same-user** process (`SIGTERM`→`SIGKILL`; `taskkill` then `/F` on Windows). Refuses other users' processes, `pid 0/1`, Aegis's own tree, and session-critical processes on every OS; binds approval to the process creation token and revalidates before each signal so PID reuse fails closed. | — |
...
The `tests/` regression suite (**827 tests**, stdlib-only, fully sandboxed — never
